exports.ADD_STUDENT_QUERY =
  ...
  "followUpDate,status,priority,createdDate,lastUpdateUser,lastUpdateTime,officeCode,currentState,studentRemarks) " +
  "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
exports.UPDATE_STUDENT_QUERY =
  ...
  "firstName=?,middleName=?,lastName=?,email=?,phoneNumber=?,additionalPhNo=?," +
  "dateOfBirth=?,gender=?,maritalStatus=?,courseInterested=?," +
  "followUpDate=?,priority=?,status=?,lastUpdateUser=?,lastUpdateTime=? ,currentState=?,studentRemarks=? " +
  "WHERE studentId= ?";

exports.UPDATE_STUDENT_FOLLOWUP_QUERY=
`UPDATE student
  SET followUpDate=? ,studentRemarks=? WHERE studentId= ?`
...
exports.FIND_STUDENT_QUERY = `SELECT 
    ...
    STD.lastUpdateUser,STD.lastUpdateTime,STD.officeCode,STD.currentState,STD.studentRemarks,EGX.overall,EGX.listening,EGX.reading,EGX.writing,EGX.speaking,
